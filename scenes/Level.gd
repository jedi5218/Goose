...
onready var robot  = get_node("Goose")
...
func _ready():
    ...
    $Goose/Spatial/Animation.play("goose_walk_loop");
...
func _physics_process(delta):
    ...
    if path.size() > 0:
        ...
        $Goose/Spatial/Animation.playback_speed = 0
...
func _unhandled_input(event):
    if event is InputEventMouseButton and event.button_index == BUTTON_LEFT and event.pressed:
        ...
        var target_point = $Navigation.get_closest_point_to_segment(from, to)
        $OccupiedHexMarker.PlaceMarker(target_point)

        # Set the path between the robots current location and our target.
        path = $Navigation.get_simple_path(robot.translation, target_point, true)
